<template>
  <div class="container-page">
     <div class="l-menu">
      ...
      <div class="flex-wrap" style="padding:20px;color:#fff;cursor:pointer;">
          <div class="l-item" @click="selected_filter=[]">显示所有</div>
      </div>
       <div class="flex-wrap" style="padding:20px;color:orange;">
          多选 <i-switch v-model="switchAssciate" style='margin-left:10px;' size="small" />
      </div>
      </div>
       <template v-for="f in filterClasses">
            <div :key='f.key' class="l-group" v-if="f.values && f.values.length">
                <div class="l-group-title"><Icon type="ios-folder" />{{f.name}}</div>
                 <template v-for="v in f.values">
                    <div :key="v.key" class="l-item" :class="isSelectFilter(f,v.key) ? 'l-item-selected':''" @click="selectFilter(f,v.key)">
                        {{v.name || v.key}} ({{v.list.length}})
                    </div>
                 </template>
            </div>
        </template>
    ...
    <div class="l-container">
        ...
        <div class="flex-between gz-card-style" style='padding:10px 10px;'>
           <div class="flex-wrap">
    ...
    cols:[{key:"id",title:"编号",
       ...
       width:120,
        ...
        option:{
          ...
          align:"center",
          buttons:[{label:"审核",
          auth:(user,item)=>(user.roles.includes('admin')),
          event:"approve"},
